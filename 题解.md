...
方法一：hash，建立一个新的类Node保存每个元素的值和出现的次数，然后使用最大堆。其中需要注意的是java中建立某个对象最大堆的方法，需要使用`PriorityQueue`然后还要给定比较大小的方法。
...
class Solution {
    ...
    public void backTrack(int p, int n, int numLeft) {
        ...
        if ((n == 0 && numLeft != 0) || (n != 0 && numLeft == 0)) {
            return;
        }
        for (int i = p; i <= 9; i++) {
            curr.add(i);
            backTrack(i+1, n-i, numLeft-1);
            curr.remove(curr.size()-1);
        }
    }
}
    ...
    public String modifyString(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (s.charAt(i) != '?') {
                ...
                if (i-1 >= 0) left = sb.charAt(i-1);
                if (i+1 < s.length()) right = s.charAt(i+1);
                for (char j = 'a'; j <= 'z';j++) {
                    if (j != left && j != right) {
                        sb.append(j);
                        break;
...
class Solution {
    public List<Double> averageOfLevels(TreeNode root) {
        ...
        int currLayerNum = 1;
        double currLayerSum = 0;
        int nextLayerNum = 0;
        int numCount = 0;
        queue.offer(root);
        while (currLayerNum != 0) {
            currLayerNum--;
            numCount++;
            TreeNode tmp = queue.poll();
            currLayerSum += tmp.val;
            if (tmp.left != null) {
                queue.offer(tmp.left);
                nextLayerNum++;
            }
            ...
            if (currLayerNum == 0) {
                currLayerNum = nextLayerNum;
                ans.add(currLayerSum / numCount);
                numCount = 0;
                nextLayerNum = 0;
                currLayerSum = 0;
            }
...
方法二：DFs，记录一个cnts,sums两个数组分别保存每一层的节点个数和val值之和。
...
class Solution {
    List<Double> ans = new LinkedList<>();
    List<Integer> cnts = new LinkedList<>();
    List<Double> sums = new LinkedList<>();
    public List<Double> averageOfLevels(TreeNode root) {
        dfs(root, 1);
        for (int i = 0; i < cnts.size(); i++) {
            ans.add(sums.get(i) / cnts.get(i));
        }
        return ans;
    }
    public void dfs(TreeNode curr, int layer) {
        if (curr == null) return;
        if (cnts.size() < layer) {
            cnts.add(1);
            sums.add((double)curr.val);
        } else {
            cnts.set(layer-1, cnts.get(layer-1)+1);
            sums.set(layer-1, sums.get(layer-1)+curr.val);
        }
        dfs(curr.left, layer+1);
        dfs(curr.right, layer+1);
    }
}
```
...
方法一：回溯（递归），这里其实不用回溯，直接递归，因为不需要再改变什么值。(超时)

时间复杂度：O(n^target) (n 是 nums个数)
...
class Solution {
    ...
    public void backTrack(int k, int[] nums, int target) {
        if (target == 0) {
            sum++;
            return;
        } 
        ...
        for (int i = k; i < nums.length; i++) {
            backTrack(0, nums, target-nums[i]);
        }
...
class Solution {
    ...
    public int helper(int[] nums, int target, int[] memo) {
        ...
        int ret = 0;
        for (int i = 0; i < nums.length; i++) {
            if(target >= nums[i]) ret += helper(nums, target - nums[i], memo);
        }
        memo[target] = ret;
        return ret;
    }
...
class Solution {
    public int breakfastNumber(int[] staple, int[] drinks, int x) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (j >= 0) ans = ans.add(BigInteger.valueOf(j+1)).mod(BigInteger.valueOf(mod));
        }
        return ans.intValue();
    }
}
...
三个状态`dp[i][0], dp[i][1], dp[i][2]`, `dp[i][0]`表示将前i个位置（包含第i）的字符全变为r的最小操作次数，`dp[i][1]`表示将前i位置变为ry这种形式的最小操作次数，`dp[i][2]`表示将前i个位置变成ryr这种形式的最小操作次数。
...
方法一：回溯。如果给定的`nums`是无重复的数组，那么我们可以用一个`isVisited`数组来记录每个位置是否被访问过。但是问题给定的数组是有重复的，所以按照上述进行的会得到重复的排列。对于这个问题，我们可以先将数组排序，让重复的数字都放在一起，那么在同一个for循环内如果当前数字和上一个数字一样，而且前一个数字没有被visit过,那么我们就跳过。
...
难点主要在于使用常数空间，需要在第一行设置一个rowFlag，因为我们要把需要置为0的列放在首行上，而如果首行本身就包含0，则会出错。
...
答：前缀和。问题很复杂很长，但本质在于看每天如果吃一颗糖和吃max的糖，在第favorDay吃到的是第几类糖。如果favorCandy在这中间，说明一定可以吃到。
...
class Solution {
    ...
    public boolean[] canEat(int[] candiesCount, int[][] queries) {
        int num_c = candiesCount.length;
        int num_q = queries.length;
        boolean[] ans = new boolean[num_q];
        // 计算前缀和
        long[] prefixSum = new long[num_c];
        ...
        for (int i = 0; i < num_q; i++) {
            ...
            if (min_eat <= prefixSum[a]) {
                if (a > 0 && (prefixSum[a-1] / c >= (b+1))) {
                    ans[i] = false;
                } else {
                    ans[i] = true;
                }
...
先计算前缀和，然后记录每个前缀和 % k的余数，如果有余数相同的且位置相隔2个及以上则存在
...
这道题也是很巧妙的使用了前缀和+hash，要先把所有的0想成是-1,然后加和。
...
方法一：这道题是一个典型的动态规划问题，要求满足的条件是所有0的个数小于等于吗，所有1的个数小于等于n,要找到最大的子集。所取的可以是不连续的，如果我们考虑所有子集的情况的话那时间复杂度太高了。我们应该从条件出发，这里的m和n都不会特别大，使用动态规划，构建数值`dp[i][j][k]`表示在第i个位置之前，满足0的个数<=j, 1的个数<=k的最大子集个数。
...
因为这里面有负数的情况，所以我们将每个数-a, a变成0, 2a。`newTarget = target + allSum`。

枚举肯定是超时的，但我们可以枚举所有的加和，也就是动态规划。`dp[i][j] = dp[i-1][j] + dp[i-1][j-nums[i-1]]`
...
其实问题可以转化成每个石子是取正还是取负加和，这样就和494的问题一样了。
...
class Solution {
    public int lastStoneWeightII(int[] stones) {
        ...
        for (int i = 1; i <= l; i++) {
            for (int j = 0; j < 2*allSum+1; j++) {
                dp[i][j] = dp[i-1][j];
                if (i > 1 && j-2*stones[i-1] >= 0) {
                    dp[i][j] += dp[i-1][j-2*stones[i-1]];
                }

                if (i == l && j >= allSum && dp[i][j] > 0) {
                    return j-allSum;
                }
            }
...
这道题要找的是盈利大于等于`minProfit`且使用人数小于等于`n`的方案总和，我们可以构造一个数组`dp[i][j][k]`表示在`i`个项目前，盈利为`j`且使用员工为`k`的方案总和，则`dp[i][j][k] = dp[i-1][j][k] + dp[i-1][j-profit[i-1]][k-profit[i-1]]`。需要注意其中除余的操作。如果这里面将`dp[i][j][k]`表示为表示在`i`个项目前，盈利至少为`j`且使用员工为`k`的方案总和的话时空复杂度会更低，需要注意。
...
本题要计算的是有多少种满足和为amount的可能，每个硬币的数量无限多。`dp[i][j]`表示的是前`i`个硬币满足和为j的所有可能，`dp[i][j] = dp[i-1][j] + dp[i-1][j-coins[i-1]] + dp[i-1][j-2*coins[i-1]]...`。

时间复杂度：O(l*amount) -> O(amount)
    ...
    public int change(int amount, int[] coins) {
        ...
        for (int i = 1; i <= l; i++) {
            for (int j = 0; j <= amount; j++) {
                dp[i][j] = dp[i-1][j];
                int tmp = j;
                while (tmp-coins[i-1] >= 0) {
                    dp[i][j] += dp[i-1][tmp-coins[i-1]];
                    tmp -= coins[i-1];
                }
            }
        }
...
方法一：动态规划，只能说跟昨天的题一摸一样。最原始的想法是构造一个`dp[i][j]`，表示在前i个数的情况下满足平方和为j的最小的个数。则`dp[i][j] = min(dp[i-1][j], dp[i-1][j-i*i]+1, dp[i-1][j-2*i*i]+2, ...)`，但是这样不仅会超时还会超内存。所以它可以进一步优化，我们可以发现`dp[i][j] = min(dp[i-1][j], dp[i][j-i*i]+1, dp[i][j-2*i*i]+2, ..., )`, 我们同时可以发现`dp[i][j-i*i] <= dp[i][j-2*i*i]+1`是必然的。所以有`dp[i][j] = min(dp[i-1][j], dp[i][j-i*i]+1)`
...
class Solution {
    ...
    public int helper(int s, int t, int[] arr) {
        int mid = (s + t) / 2;
        if (arr[mid] <= arr[mid+1]) {
            return helper(mid+1, t, arr);
        } else {
            if (arr[mid] <= arr[mid-1]) {
                return helper(s, mid-1, arr);
            } else {
                return mid;
            }
        }
...
典型的动态规划，`dp[i][j]`表示在`[i,j]`之间alex能得到的最大值，则`dp[i][j] = Math.max( cnt[i+1][j]-dp[i+1][j]+piles[i], cnt[i][j-1]-dp[i][j-1]+piles[j] );`
...
class Solution {
    public boolean stoneGame(int[] piles) {
        ...
        for (int k = 1; k < n; k++) {
            for (int i = 0; i+k < n; i++) {
                int j = i+k;
                dp[i][j] = Math.max( cnt[i+1][j]-dp[i+1][j]+piles[i], cnt[i][j-1]-dp[i][j-1]+piles[j] );
            }
        }
        if (2 * dp[0][n-1] > cnt[0][n-1])
            return true;
        else
            return false;
    }
...
`n = 1 + x + x^2+...x^k`，我们目标是找到最小的`x`满足前面的式子，那么`x`越小`k`越大，那么最大最大`x=n-1`。因为给定了`n`的范围`[0, 10e18]`，所以我们可以得出当`x=2`时，`k`最大取64，也就是`k`的范围我们可以知道是`[1, 64]`，然后对每个`k`我们遍历x，从`[1, n^(1/k)]`进行二分。
...
class Solution {
    public String smallestGoodBase(String n) {
        ...
        for (int k = kMax; k >= 2; k--) {
            int s = 2;
            int t = (int) Math.floor(Math.pow(number, 1.0/k)); 

            while (s <= t) {
                int mid = (s + t) / 2;
                long tmp = helper(mid, k);
                if (tmp == number) {
    ...
    public long helper(int x, int k) {
        long ans = 1;
        for (int i = 0; i < k; i++) {
            ans = ans * x + 1;
        }
...
此题不能背包，只能回溯求解。构造一个`state`数组，里面存放的每个字符当前是否在字符串中出现。
...
class Solution {
    ...
    public int maxLength(List<String> arr) {
        ...
        StringBuffer sb = new StringBuffer();
        backtrack(0, state, arr, 0);
        return maxL; 
    }

    public void backtrack(int i, char[] state, List<String> arr, int currL) {
        ...
        if (flag == 1) {
            if (maxL < currL+tmp.length()) {
                maxL = currL + tmp.length();
            }
            pos = tmp.length();

            backtrack(i+1, state, arr, currL+tmp.length());
        }
...
方法一：构造一个`<parent, childs>`的map，以及一个`death`的map，里面存的是所有死亡的孩子
...
方法一：回溯，用一个`hMap`表示hour的取了各个1的List，`mMap`表示minite取了各个1的List。
...
class Solution {
    public List<String> readBinaryWatch(int turnedOn) {
        ...
        helper(0, 0, hMap, Math.min(turnedOn, 4), 0, 4);
        helper(0, 0, mMap, Math.min(turnedOn, 6), 0, 6);

        for (int i = 0; i <= Math.min(turnedOn, 4); i++) {
            int j = turnedOn - Math.min(i, 4);
            List<Integer> hList = hMap.get(i);
            List<Integer> mList = mMap.get(j);
            int hour, mini;
            if (hList == null || mList == null) {
                continue;
            }
            ...
            for (int m = 0; m < hList.size(); m++) {
                ...
                for (int n = 0; n < mList.size(); n++) {
                    mini = mList.get(n);
                    if (mini < 10) {
                        ans.add(String.valueOf(hour) + ":0" + String.valueOf(mini));
                    } else {
                        ans.add(String.valueOf(hour) + ":" + String.valueOf(mini));
                    }
                }
            }
        }
    ...
    public void helper(int i, int currSum, HashMap<Integer, List<Integer>> hMap, int cntLength, int cnt, int length) {
        if (cnt > cntLength || i > length) return;
        List<Integer> tmp;
        if (!hMap.containsKey(cnt)) {
            tmp = new LinkedList<>();
            if (length == 6 && currSum <= 59) {
                tmp.add(currSum);
            } else if (length == 4 && currSum <= 11) {
                tmp.add(currSum);
            } else {
                return;
            }
        ...
        helper(i+1, currSum, hMap, cntLength, cnt+1, length);

        helper(i+1, currSum-(int)Math.pow(2, i), hMap, cntLength, cnt, length);
...
class Solution {
    ...
    public void helper(StringBuffer sb, List<Character> charList) {
...
给定一个字符串，可以将其分割成任意块，问使得其分割的各个块的数均为质数且每块开头不能为0的可能数, 已知质数范围为[1, 1e6]
...
public class Main {
    ...
    public class Solution {
        ...
        public int helper(String number, int i) {
            int l = number.length();
            if (i >= l) return 1;
            if (number.charAt(i) == '0') return 0;
            int ans = 0;
            for (int k = 1; k <= 6; k++) {
                if (i+k > l) break;
...
```java
class Solution {
    HashMap<String, Integer> deadMap;
    ...
    public int openLock(String[] deadends, String target) {
        ...
        for (int i = 0; i < deadends.length; i++) {
            deadMap.put(deadends[i], 0);
        }

        if (deadMap.containsKey("0000")) return -1;

        queue.offer(new Pair<String,Integer>("0000",0));
        deadMap.put("0000", 0);
        while (!queue.isEmpty()) {
            Pair<String, Integer> curr = queue.poll();
            if (curr.getKey().equals(target)) return curr.getValue();
            StringBuffer sb = new StringBuffer(curr.getKey());
            for (int i = 0; i < 4; i++) {
                char origin = sb.charAt(i);
                char c = lastChar(origin);
                for (int j = 0; j < 2; j++) {
                    sb.setCharAt(i, c);
                    String tmp = sb.toString();
                    if (!deadMap.containsKey(tmp)) {
                        queue.offer(new Pair<String, Integer>(tmp, curr.getValue() + 1));
                        deadMap.put(tmp, 0);
                    }
                    c = nextChar(origin);
                }
    ...
    public char lastChar(char c) {
        if (c == '0') {
            return '9';
        } else {
            int tmp = (c - '0') - 1;
            return Integer.toString(tmp).charAt(0);
        }
    }
...
class Solution {
    ...
    int[] zeroPos = new int[]{0, 0};
    int[][] urdl = new int[][]{{-1, 0}, {0, 1}, {1, 0}, {0, -1}};
    public int slidingPuzzle(int[][] board) {
        visited = new HashMap<>();
        queue = new LinkedList<>();
        if (isSatisfied(board, 0)) {
            return 0;
        }
        
        while (!queue.isEmpty()) {
            Pair<String, Integer> curr = queue.poll();
            int[][] currArray = string2Array(curr.getKey());
            for (int i = 0; i < 4; i++) {
                int[] o = urdl[i];
                int[] swapPos = new int[2];
                swapPos[0] = zeroPos[0] + o[0];
                swapPos[1] = zeroPos[1] + o[1];
                if ((swapPos[0] >= 0 && swapPos[0] <= 1) && (swapPos[1] >= 0 && swapPos[1] <= 2)) {
                    swap(currArray, zeroPos, swapPos);
                    if (isSatisfied(currArray, curr.getValue() + 1))    return curr.getValue() + 1;
                    swap(currArray, swapPos, zeroPos);
                }
            }
        }
        return -1;
    ...
    public boolean isSatisfied(int[][] curr, int depth) {
        boolean ans = true;
        StringBuffer sb = new StringBuffer();
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 3; j++) {
                int c = curr[i][j];
                sb.append(c);
                if (c != end[i][j]) ans = false;
            }
        }
        String s = sb.toString();
        if (!visited.containsKey(s)) {
            visited.put(s, depth);
            queue.offer(new Pair<String, Integer>(s, depth));
        } 
        return ans;
    }
    ...
    public void swap(int[][] curr, int[] x, int[] y) {
        int tmp = curr[x[0]][x[1]];
        curr[x[0]][x[1]] = curr[y[0]][y[1]];
        curr[y[0]][y[1]] = tmp;
    }
}
...
class Solution {
    ...
    public int snakesAndLadders(int[][] board) {
        ...
        queue.offer(new Pair<Integer, Integer>(1, 0));
        int[] pos = index2Pos(1);
        visited[pos[0]][pos[1]] = 1;

        while (!queue.isEmpty()) {
            Pair<Integer, Integer> currPair = queue.poll();
            int curr = currPair.getKey();
            if (curr == m*n) return currPair.getValue();
            pos = index2Pos(curr);
            for (int i = 1; i <= 6; i++) {
                int next = curr + i;
                if (next <= m * n) {
                    ...
                    if (visited[x][y] != 1) {
                        if (board[x][y] == -1) {
                            visited[x][y] = 1;
                            queue.offer(new Pair<Integer, Integer>(next, currPair.getValue()+1));
                        } else {
                            visited[x][y] = 1;
                            pos = index2Pos(board[x][y]);
                            if (visited[pos[0]][pos[1]] == 0) {
                                visited[pos[0]][pos[1]] = 2;
                                queue.offer(new Pair<Integer, Integer>(board[x][y], currPair.getValue()+1));
                            }
